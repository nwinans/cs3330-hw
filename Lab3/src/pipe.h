...
typedef struct Pipe_Op {
    ...
    /* predicted branch information */
    int pred_is_branch;   /* is this a branch (conditional or unconditional)? */
    int pred_is_cond;     /* is this a conditional branch? */
    uint32_t pred_branch_dest; /* what was the predicted next PC? */
    int pred_branch_taken; /* was this branch predicted taken? */
