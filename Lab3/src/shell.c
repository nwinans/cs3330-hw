...
uint32_t stat_correct_predict = 0, stat_all_predict = 0;
...
void rdump() {
    ...
    printf("Flushes: %u\n", stat_squash);
    printf("BPAccuracy: %0.3f\n", ((float) stat_correct_predict) / stat_all_predict);
}
...
void get_command() {
  ...
  case 'p':
   printf("FETCH: PC %x\n", pipe.PC);
   printf("DECODE: PC %x\n", pipe.decode_op ? pipe.decode_op->pc : 0);
   printf("EXECUTE: PC %x\n", pipe.execute_op ? pipe.execute_op->pc : 0);
   printf("MEM: PC %x\n", pipe.mem_op ? pipe.mem_op->pc : 0);
   printf("WB: PC %x\n", pipe.wb_op ? pipe.wb_op->pc : 0);
   break;
